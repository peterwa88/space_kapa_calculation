...
g = read_vizier(RAW / "cf3_table2_all.tsv")
...
t = read_vizier(RAW / "cf3_table3_all.tsv")
...
for label, df in [("primary", primary), ("strict", strict), ("sn_only", sn)]:
    ...
    out["resid_logL"] = xr
    out["resid_logH"] = yr
    out.to_csv(DERIVED / f"analysis_{label}.csv", index=False)

(RESULTS / "analysis_results.json").write_text(json.dumps(results, indent=2, default=float), encoding="utf-8")
